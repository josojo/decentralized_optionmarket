...
//logic needs to be updated in the file, right now just coopy of opitonput.sol
import "Collateral.sol";
import "OptionTradeCall.sol";
import "Option.sol";

contract OptionCall is Option{

		bool optionCallSet=false;
	function setOptionCall(address marketPairadd2, address coladd,uint expiration,uint strikeP,uint tf){//, uint expriation, uint strikeP, bool type
		if(optionCallSet)throw;
	...
	function takeBid(uint id, uint ind, uint amount){
		...
		if(msg.value< amount*optionbids[ind].price/pcollateral+tradingfee)throw;
			...
			if(amount>insuranceamount){
				...
				trades[tradesnr]=new OptionTradeCall(marketPairadd,collateraladd,msg.sender,optionbids[ind].owner,amount,strikePrice,expirationDateinblocks);				trades[tradesnr].send(1 ether);
